...
namespace flexflow {
  ...
  cudnnStatus_t cudnnSetTensorDescriptorFromDomain(cudnnTensorDescriptor_t tensor, Domain domain)
  {
    ...
    for (int i = 0; i < domain.get_dim(); i++) {
      int dim_idx = domain.get_dim() - i - 1;
      dims[i] = hi[dim_idx] - lo[dim_idx] + 1;
    }
    return cudnnSetTensor4dDescriptor(tensor, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, dims[0], dims[1], dims[2], dims[3]);
  }
