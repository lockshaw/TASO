...
cmake -DProtobuf_INCLUDE_DIR="$PROTOBUF/src/" -DProtobuf_PROTOC_EXECUTABLE="$PROTOBUF/src/.libs/protoc" -DCUDA_CUDNN_LIBRARY='/share/software/user/open/cudnn/7.6.5/lib64/libcudnn.so' -DProtobuf_LIBRARY="$PROTOBUF/src/.libs/libprotobuf.so" -DBUILD_CPP_EXAMPLES=ON -DBUILD_TESTS=ON -DCMAKE_BUILD_TYPE=Debug -DCMAKE_BUILD_TYPE=Debug ..
