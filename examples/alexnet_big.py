...
def classifier(graph, input, num_classes):
    input = linear(graph, input, 4096 * 4)
    input = relu(graph, input)
    input = linear(graph, input, 4096 * 4)
    input = relu(graph, input)
...
DEFAULT_MODEL_PATH = Path("/home/groups/aaiken/unger/models/alexnet-big")
...
if __name__ == "__main__":
    ...
    print('Converting optimized')
    unoptimized_model = ts.export_onnx(graph)
    print('Finished converting optimized')
    debug_dir = None
    ...
    if args.export:
        print('Checking unoptimized')
        onnx.checker.check_model(unoptimized_model)
        print('Exporting unoptimized')
        onnx.save(unoptimized_model, str(args.output_dir / f"alexnet-{args.budget}x{ff_budget}_{batch_size}_unoptimized_n{args.num_classes}_alpha{args.alpha}.onnx"))
    print('Optimizing')
    ...
    if args.export:
        ...
        print('Exporting optimized')
        onnx.save(optimized_model, str(args.output_dir / f"alexnet-{args.budget}x{ff_budget}_{batch_size}_optimized_n{args.num_classes}_alpha{args.alpha}.onnx"))
